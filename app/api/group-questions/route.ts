...
[그룹 묶기 규칙 — 매우 중요]
- **핵심 키워드(주제어)가 동일하거나 직접적으로 연관된 질문만** 같은 그룹으로 묶으세요.
- 예: "스킬이 뭐야?" + "스킬이 머예여?" → 같은 그룹 (둘 다 '스킬' 주제)
- 예: "PM이 뭐에요?" + "스킬이 뭐야?" → **다른 그룹** (PM ≠ 스킬)
- 예: "PM이 뭐에요?" + "PM 어떻게 해?" → 같은 그룹 (둘 다 'PM' 주제)
- 단순히 짧거나 비슷한 문장 구조라는 이유만으로 묶지 마세요.
- 확신이 없으면 별도 그룹으로 분리하세요. 잘못 묶는 것보다 너무 많이 나뉘는 게 낫습니다.`,
        },
        {
          role: 'user',
          content: `아래 메시지 목록에서 진짜 질문만 골라 그룹으로 묶고, JSON 형식으로만 응답하세요.
질문이 아닌 메시지는 완전히 무시하고 groups에 포함하지 마세요.
핵심 주제가 다른 질문은 반드시 별도 그룹으로 분리하세요.
